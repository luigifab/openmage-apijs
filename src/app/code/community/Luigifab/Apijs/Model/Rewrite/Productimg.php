	...
	public function beforeSave($object) {
		...
		if (is_array($value)) {
			...
			if (!empty($value['delete']))
				$help->removeFiles($help->getCatalogProductImageDir(), basename($value['value'])); // pas uniquement dans le cache
			else if (!empty($value['value']) && !empty($_FILES[$name]['size']))
				$help->removeFiles($help->getCatalogProductImageDir(), basename($value['value']), true); // pas uniquement dans le cache
		}
