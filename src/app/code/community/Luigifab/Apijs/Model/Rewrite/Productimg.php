		...
		if (is_array($value) && !empty($value['value'])) {
			...
			if ($count == 1) {
				...
				if (!empty($value['delete']))
					$help->removeFiles($help->getCatalogProductImageDir(), basename($file)); // everywhere (not only in cache dir)
				else if (!empty($_FILES[$name]['size']))
					$help->removeFiles($help->getCatalogProductImageDir(), basename($file), true); // everywhere (not only in cache dir)
			}
		}
