...
if((typeof apijs.core.i18n==='function')&&(typeof apijs.core.bbcode==='function')&&(typeof apijs.core.dialog==='function')&&(typeof apijs.core.upload==='function')&&(typeof apijs.core.slideshow==='function')){apijs.i18n=new apijs.core.i18n();if(typeof setApijsLang==='function')
setApijsLang();apijs.i18n.init();if(typeof setApijsConfig==='function')
...
function openTab(ev){ev.preventDefault();window.open(this.href);}
...
apijs.config.lang='en';};this.translate=function(word){var lang=apijs.config.lang;if(typeof this.data[lang][word]!=='string'){if((lang!=='en')&&(typeof this.data['en'][word]==='string'))
lang='en';else
return word;}
if(arguments.length>1){var i=0,arg=1,data,translation='';for(data=this.data[lang][word].split('§');i<data.length;i++)
translation+=(arg<arguments.length)?data[i]+arguments[arg++]:data[i];return translation;}
return this.data[lang][word];};this.changeLang=function(lang){if(lang.indexOf('auto')>-1){apijs.config.lang=lang;this.init();return true;}
if((typeof lang==='string')&&this.data.hasOwnProperty(lang)){apijs.config.lang=lang;return true;}
return false;};};
...
this.emotes=(typeof emotes==='boolean')?emotes:true;};this.getFragment=function(){this.readData(this.bbcode,0);this.fragment=document.createDocumentFragment();this.fragment.appendChild(this.createDomFragment(this.object));return this.fragment;};this.readData=function(data,level){var element,attributes,content,text,other,cut;if((data[0]!=='[')&&((cut=data.search(/\[([a-z1-6]+)(?: [a-z:]+=["'][^\]]*["'])*\]/))>-1)){text=data.slice(0,cut);this.readData(text,level);other=data.slice(cut);if((cut=other.indexOf('[/'+RegExp.$1+']'))>-1){element=other.slice(0,cut+RegExp.$1.length+3);other=other.slice(cut+RegExp.$1.length+3);this.readData(element,level);}
...
this.readData(other,level);}
else{this.addElement(data,null,level);}};this.addElement=function(data,attributes,level){var directlink,attribute,name,value,hasEmotes=false,currentEmote={};directlink=this.getContentNode(this.object,0,level);if(attributes!==null){if(directlink.hasOwnProperty('content'))
directlink.content.push({tag:data});else
directlink.content=[{tag:data}];if(attributes.length>5){attributes=attributes.slice(1,-1).split(/["'] /);for(attribute in attributes)if(attributes.hasOwnProperty(attribute)){name=attributes[attribute].slice(0,attributes[attribute].indexOf('='));value=attributes[attribute].slice(attributes[attribute].indexOf('=')+2);if(directlink.hasOwnProperty('content'))
...
return this.getContentNode(dom.content[i],level,maxlevel);return dom.content[i];};this.createDomFragment=function(data){var tag,attr,elem;if(data.hasOwnProperty('tag'))
...
if(apijs.config.dialog.savingTime>500){this.timerbis=window.setTimeout(function(){if(typeof this.callback==='function'){this.classNames=this.classNames.replace('lock','');this.callback(this.callbackParam);}}.bind(this),apijs.config.dialog.savingTime);}
else{this.classNames=this.classNames.replace('lock','');this.callback(this.callbackParam);}
result=true;}
else if(this.classNames.indexOf('options')>-1){if(this.callback(this.callbackParam)===true){this.classNames+=' lock';this.elemA=document.createElement('p');this.elemA.setAttribute('class','saving');this.elemA.appendChild(document.createTextNode(apijs.i18n.translate('operationInProgress')));document.getElementById('box').removeChild(document.getElementById('box').lastChild);document.getElementById('box').lastChild.setAttribute('class','novisible');document.getElementById('box').appendChild(this.elemA);this.timer=window.setTimeout(apijs.dialog.htmlLinkReload.bind(this),10000);if(apijs.config.dialog.savingTime>500){this.timerbis=window.setTimeout(function(){if(document.getElementById('box')&&(document.getElementById('box').nodeName.toLowerCase()==='form')){this.classNames=this.classNames.replace('lock','');document.getElementById('box').submit();}}.bind(this),apijs.config.dialog.savingTime);}
else{result=true;}}}
else if(this.classNames.indexOf('upload')>-1){if((apijs.upload.extensions instanceof Array)&&(apijs.upload.actionConfirm()===true)){this.classNames+=' lock';result=true;}
...
if(in_array(['waiting','progress','lock'],classNames)){ev.preventDefault();ev.stopPropagation();return'';}};this.actionResizeBrowser=function(){var infoWindow={},infoMedia={},imageSize=apijs.dialog.imageSize;infoMedia.width=parseInt(document.getElementById('topho').getAttribute('width'),10);infoMedia.height=parseInt(document.getElementById('topho').getAttribute('height'),10);infoWindow.width=Math.round(window.innerWidth-window.innerWidth*apijs.config.dialog.margin/100);infoWindow.height=Math.round(window.innerHeight-window.innerHeight*apijs.config.dialog.margin/100);if((infoMedia.width>infoWindow.width)||(infoMedia.width<infoWindow.width)){infoMedia.width=infoWindow.width;infoMedia.height=Math.round(infoMedia.width/imageSize.ratio);if(infoMedia.height>infoWindow.height){infoMedia.height=infoWindow.height;infoMedia.width=Math.round(infoMedia.height*imageSize.ratio);}}
else if((infoMedia.height>infoWindow.height)||(infoMedia.height<infoWindow.height)){infoMedia.height=infoWindow.height;infoMedia.width=Math.round(infoMedia.height*imageSize.ratio);if(infoMedia.width>infoWindow.width){infoMedia.width=infoWindow.width;infoMedia.height=Math.round(infoMedia.width/imageSize.ratio);}}
if((infoMedia.width>imageSize.width)||isNaN(infoMedia.width)||(infoMedia.width<1)){infoMedia.width=imageSize.width;infoMedia.height=imageSize.height;}
else if((infoMedia.height>imageSize.height)||isNaN(infoMedia.height)||(infoMedia.height<1)){infoMedia.width=imageSize.width;infoMedia.height=imageSize.height;}
document.getElementById('box').style.width=infoMedia.width+'px';document.getElementById('box').style.marginLeft=Math.round(-(infoMedia.width+20)/2)+'px';document.getElementById('box').style.marginTop=Math.round(-(infoMedia.height+66)/2)+'px';document.getElementById('topho').setAttribute('width',infoMedia.width);document.getElementById('topho').setAttribute('height',infoMedia.height);if(document.getElementById('topho').querySelector('div.bbcode'))
document.getElementById('topho').querySelector('div.bbcode').style.height=infoMedia.height+'px';};this.actionPauseVideo=function(){var hidden;if(typeof document.hidden==='boolean')
...
hidden=document.webkitHidden;if(hidden){apijs.dialog.videoPlaying=!document.getElementById('topho').paused;document.getElementById('topho').pause();}
else{if(apijs.dialog.videoPlaying)
document.getElementById('topho').play();}};this.restrictNavigation=function(lock){var tags='a,area,button,input,object,select,textarea,iframe',elems=document.querySelectorAll(tags),elem;if(lock){for(elem=0;elem<elems.length;elem++)
elems[elem].setAttribute('tabindex','-1');}
else{for(elem=0;elem<elems.length;elem++)
elems[elem].removeAttribute('tabindex');}};this.updateWindowSize=function(width,height,url){var infoMedia={width:width,height:height,id:url.slice((url.lastIndexOf('/')+1),url.lastIndexOf('.'))},infoWindow={width:window.innerWidth,height:window.innerHeight};this.imageSize={width:width,height:height,ratio:width/height};if((width>=Math.round(window.innerWidth-window.innerWidth*apijs.config.dialog.margin/100))||(height>=Math.round(window.innerHeight-window.innerHeight*apijs.config.dialog.margin/100))){infoWindow.width-=Math.round(window.innerWidth*apijs.config.dialog.margin/100);infoWindow.height-=Math.round(window.innerHeight*apijs.config.dialog.margin/100);if(infoMedia.width>infoWindow.width){infoMedia.width=infoWindow.width;infoMedia.height=Math.round(infoMedia.width/this.imageSize.ratio);if(infoMedia.height>infoWindow.height){infoMedia.height=infoWindow.height;infoMedia.width=Math.round(infoMedia.height*this.imageSize.ratio);}}
else if(infoMedia.height>infoWindow.height){infoMedia.height=infoWindow.height;infoMedia.width=Math.round(infoMedia.height*this.imageSize.ratio);if(infoMedia.width>infoWindow.width){infoMedia.width=infoWindow.width;infoMedia.height=Math.round(infoMedia.width/this.imageSize.ratio);}}}
this.fragment.firstChild.firstChild.style.width=infoMedia.width+'px';this.fragment.firstChild.firstChild.style.marginLeft=Math.round(-(infoMedia.width+20)/2)+'px';this.fragment.firstChild.firstChild.style.marginTop=Math.round(-(infoMedia.height+66)/2)+'px';return infoMedia;};this.searchMimeType=function(file){var ext,types={jpg:'image/jpeg',jpeg:'image/jpeg',png:'image/png',gif:'image/gif',svg:'image/svg+xml',ogv:'video/ogg',webm:'video/webm',mp4:'video/mp4',m4v:'video/mp4'};ext=file.slice(file.lastIndexOf('.')+1,(file.lastIndexOf('#')>0)?file.lastIndexOf('#'):file.length);return(types.hasOwnProperty(ext))?types[ext]:'';};this.loadImage=function(width,height,url){if(apijs.config.navigator&&apijs.config.dialog.showLoader){this.image=new Image(width,height);this.image.src=url;this.image.addEventListener('load',function(){if(document.getElementById('topho')){document.getElementById('topho').removeAttribute('class');document.getElementById('topho').setAttribute('src',apijs.dialog.image.src);}}.bind(this),false);this.image.addEventListener('error',function(){window.setTimeout(function(){if(document.getElementById('topho')){if(document.getElementById('topho').getAttribute('class').indexOf('resized')>-1){var img=document.getElementById('topho').cloneNode(true);document.getElementById('topho').parentNode.parentNode.removeChild(document.getElementById('topho').parentNode);document.getElementById('box').firstChild.firstChild.appendChild(img);}
if(apijs.config.dialog.savePhoto)
document.getElementById('box').firstChild.lastChild.removeChild(document.getElementById('box').firstChild.lastChild.lastChild);document.getElementById('topho').setAttribute('class',document.getElementById('topho').getAttribute('class').replace('loading','error')+' '+apijs.config.lang);}},750);}.bind(this),false);}
else{document.getElementById('topho').removeAttribute('class');document.getElementById('topho').setAttribute('src',url);}};this.setupDialog=function(type,icon){if(this.classNames!==null){this.deleteDialog(false);}
...
if(apijs.config.navigator){if(apijs.config.dialog.closeOnClic&&!in_array(['waiting','progress','lock'],classNames))
document.addEventListener('click',apijs.dialog.actionCloseOnClic,false);if(in_array(['photo','video'],classNames))
window.addEventListener('resize',apijs.dialog.actionResizeBrowser,false);if(in_array('video',classNames)){if(typeof document.hidden==='boolean')
...
document.addEventListener('webkitvisibilitychange',apijs.dialog.actionPauseVideo,false);}}};this.deleteDialog=function(total){var classNames=this.classNames.split(' '),video=document.getElementById('topho');if(this.timer)
clearTimeout(this.timer);if(apijs.config.navigator){document.removeEventListener('keydown',apijs.dialog.actionKey,true);window.removeEventListener('beforeunload',apijs.dialog.actionCloseBrowser,true);this.restrictNavigation(false);if(video&&(typeof video.pause==='function'))
video.pause();if(apijs.config.dialog.closeOnClic&&!in_array(['waiting','progress','lock'],classNames))
document.removeEventListener('click',apijs.dialog.actionCloseOnClic,false);if(in_array(['photo','video'],classNames))
window.removeEventListener('resize',apijs.dialog.actionResizeBrowser,false);if(in_array('video',classNames)){if(typeof document.hidden==='boolean')
...
if(total&&(!apijs.config.transition||in_array('notransition',classNames))){document.getElementById('dialog').parentNode.removeChild(document.getElementById('dialog'));}
...
this.elemA.setAttribute('src',apijs.config.dialog.imageUpload.src);this.elemA.setAttribute('sandbox','allow-forms allow-same-origin');this.elemA.setAttribute('name','iframeUpload.'+uploadkey);this.elemA.setAttribute('style','display:none;');this.elemA.setAttribute('id','iframeUpload');document.body.appendChild(this.elemA);this.elemA=document.createElement('div');this.elemB=document.createElement('input');this.elemB.setAttribute('type','hidden');this.elemB.setAttribute('name','UPLOAD_PROGRESS');this.elemB.setAttribute('value',uploadkey);this.elemA.appendChild(this.elemB);this.elemB=document.createElement('input');this.elemB.setAttribute('type','file');this.elemB.setAttribute('name',inputname);this.elemB.setAttribute('onchange',"document.getElementById('box').lastChild.firstChild.focus();");this.elemA.appendChild(this.elemB);this.fragment.firstChild.firstChild.appendChild(this.elemA);};this.htmlProgressBar=function(){if(apijs.config.navigator){this.elemA=document.createElement('object');this.elemA.setAttribute('data',apijs.config.dialog.imageUpload.src);this.elemA.setAttribute('type','image/svg+xml');this.elemA.setAttribute('width',apijs.config.dialog.imageUpload.width);this.elemA.setAttribute('height',apijs.config.dialog.imageUpload.height);this.elemA.setAttribute('id','progressbar');this.fragment.firstChild.firstChild.appendChild(this.elemA);}
else if(!apijs.config.navigator){this.elemA=document.createElement('embed');this.elemA.setAttribute('src',apijs.config.dialog.imageUpload.src);this.elemA.setAttribute('type','image/svg+xml');this.elemA.setAttribute('wmode','transparent');this.elemA.setAttribute('width',apijs.config.dialog.imageUpload.width);this.elemA.setAttribute('height',apijs.config.dialog.imageUpload.height);this.elemA.setAttribute('id','progressbar');this.fragment.firstChild.firstChild.appendChild(this.elemA);}};this.htmlPhoto=function(width,height,url,name,date,legend){var media=this.updateWindowSize(width,height,url);this.elemA=document.createElement('dl');this.elemB=document.createElement('dt');if(apijs.config.dialog.showFullsize&&((media.width!==width)||(media.height!==height))){this.elemC=document.createElement('a');this.elemC.setAttribute('href',url);this.elemC.setAttribute('onclick','window.open(this.href); this.blur(); return false;');this.elemD=document.createElement('img');this.elemD.setAttribute('width',media.width);this.elemD.setAttribute('height',media.height);this.elemD.setAttribute('alt','');if(document.implementation.hasFeature('http://www.w3.org/TR/SVG11/feature#Image','1.1'))
this.elemD.setAttribute('class','loading svg resized');else
this.elemD.setAttribute('class','loading resized');this.elemD.setAttribute('id','topho');this.elemC.appendChild(this.elemD);this.elemC.appendChild(document.createElement('span'));}
else{this.elemC=document.createElement('img');this.elemC.setAttribute('width',media.width);this.elemC.setAttribute('height',media.height);this.elemC.setAttribute('alt','');if(document.implementation.hasFeature('http://www.w3.org/TR/SVG11/feature#Image','1.1'))
this.elemC.setAttribute('class','loading svg');else
this.elemC.setAttribute('class','loading');this.elemC.setAttribute('id','topho');}
...
this.elemC.appendChild(document.createTextNode(media.id+' ('+date+')'));else if(name==='auto')
this.elemC.appendChild(document.createTextNode(media.id));else if(date!=='false')
this.elemC.appendChild(document.createTextNode('('+date+')'));this.elemB.appendChild(this.elemC);}
this.elemB.appendChild(document.createTextNode(' '+legend+' '));if(apijs.config.dialog.savePhoto){this.elemC=document.createElement('a');this.elemC.setAttribute('href',apijs.config.dialog.filePhoto+'?id='+media.id);this.elemC.setAttribute('type',this.searchMimeType(url));this.elemC.setAttribute('class','download');this.elemC.appendChild(document.createTextNode(apijs.i18n.translate('downloadLink')));this.elemB.appendChild(this.elemC);}
this.elemA.appendChild(this.elemB);this.fragment.firstChild.firstChild.appendChild(this.elemA);};this.htmlVideo=function(url,name,date,legend){var i,multiple,noVideo,mimetype=this.searchMimeType(url),media=this.updateWindowSize(apijs.config.dialog.videoWidth,apijs.config.dialog.videoHeight,url);this.elemA=document.createElement('dl');this.elemB=document.createElement('dt');this.elemC=document.createElement('video');this.elemC.setAttribute('width',media.width);this.elemC.setAttribute('height',media.height);this.elemC.setAttribute('controls','controls');if(apijs.config.dialog.videoAutoplay)
this.elemC.setAttribute('autoplay','autoplay');this.elemC.setAttribute('id','topho');if(url.indexOf('#')>0){multiple=url.slice(url.indexOf('#')+1);multiple=multiple.split(',');multiple.unshift(url.slice(url.lastIndexOf('.')+1,url.lastIndexOf('#')));for(i=0;i<multiple.length;i++){this.elemD=document.createElement('source');this.elemD.setAttribute('src',url.slice(0,url.lastIndexOf('.'))+'.'+multiple[i]);this.elemD.setAttribute('type',this.searchMimeType(multiple[i]));this.elemC.appendChild(this.elemD);}}
else{this.elemD=document.createElement('source');this.elemD.setAttribute('src',url);this.elemD.setAttribute('type',mimetype);this.elemC.appendChild(this.elemD);}
if(typeof this.elemC.pause!=='function'){noVideo=new apijs.core.bbcode();noVideo.init(apijs.i18n.translate('browserNoVideo',BrowserDetect.browser,BrowserDetect.version,mimetype,navigator.userAgent));noVideo=noVideo.getFragment();noVideo.firstChild.setAttribute('style','height:'+media.height+'px');this.elemC.appendChild(noVideo);}
this.elemB.appendChild(this.elemC);this.elemA.appendChild(this.elemB);this.elemB=document.createElement('dd');if((name!=='false')||(date!=='false')){this.elemC=document.createElement('span');if((name!=='false')&&(name!=='auto')&&(date!=='false'))
this.elemC.appendChild(document.createTextNode(name+' ('+date+')'));else if((name!=='false')&&(name!=='auto'))
this.elemC.appendChild(document.createTextNode(name));else if((name==='auto')&&(date!=='false'))
this.elemC.appendChild(document.createTextNode(media.id+' ('+date+')'));else if(name==='auto')
this.elemC.appendChild(document.createTextNode(media.id));else if(date!=='false')
this.elemC.appendChild(document.createTextNode('('+date+')'));this.elemB.appendChild(this.elemC);}
this.elemB.appendChild(document.createTextNode(' '+legend+' '));if(apijs.config.dialog.saveVideo){this.elemC=document.createElement('a');this.elemC.setAttribute('href',apijs.config.dialog.fileVideo+'?id='+media.id);this.elemC.setAttribute('type',mimetype);this.elemC.setAttribute('class','download');this.elemC.appendChild(document.createTextNode(apijs.i18n.translate('downloadLink')));this.elemB.appendChild(this.elemC);}
...
apijs.core.upload=function(){this.percent=0;this.maxsize=0;this.extensions=null;this.uploadkey=null;this.callback=null;this.callbackParam=null;this.svgTimer=null;this.svgDirection=0;this.svgWaiting=0;this.progressTimer=null;this.progressWaiting=0;this.sendFile=function(title,inputname,maxsize,extensions,callback,callbackParam,icon){if((typeof title==='string')&&(typeof inputname==='string')&&(typeof maxsize==='number')&&(typeof extensions==='object')&&(extensions instanceof Array)&&(typeof callback==='function')){this.callback=callback;this.callbackParam=callbackParam;this.extensions=extensions;this.uploadkey=uniqid();this.maxsize=maxsize;apijs.dialog.dialogFormUpload(title,this.prepareText(maxsize),inputname,this.uploadkey,icon);}
else if(apijs.config.debug){if((typeof callback==='function')&&(typeof callback.name==='string')&&(callback.name.length>0))
callback=callback.name;apijs.dialog.dialogInformation(apijs.i18n.translate('debugInvalidCall'),'[pre]TheUpload » sendFile[br]➩ (string) title : '+title+'[br]➩ (string) inputname : '+inputname+'[br]➩ (number) maxsize : '+maxsize+'[br]➩ (array) extensions : '+extensions+'[br]➩ (function) callback : '+callback+'[br]➩ (mixed) callbackParam : '+callbackParam+'[br]➩ (string) icon : '+icon+'[/pre]');}};this.deleteFile=function(title,text,fileid,callback,callbackParam,icon){if((typeof title==='string')&&(typeof text==='string')&&(typeof fileid==='string')&&(typeof callback==='function')){this.callback=callback;this.callbackParam=callbackParam;icon=(typeof icon!=='string')?'delete':icon;apijs.dialog.dialogConfirmation(title,text,apijs.upload.actionDelete,fileid,icon);}
else if(apijs.config.debug){if((typeof callback==='function')&&(typeof callback.name==='string')&&(callback.name.length>0))
callback=callback.name;apijs.dialog.dialogInformation(apijs.i18n.translate('debugInvalidCall'),'[pre]TheUpload » deleteFile[br]➩ (string) title : '+title+'[br]➩ (string) text : '+text+'[br]➩ (string) fileid : '+fileid+'[br]➩ (function) callback : '+callback+'[br]➩ (mixed) callbackParam : '+callbackParam+'[br]➩ (string) icon : '+icon+'[/pre]');}};this.prepareText=function(maxsize){var text,last=this.extensions[this.extensions.length-1];if(last==='*')
text=apijs.i18n.translate('uploadAllType',maxsize);else if(this.extensions.length===1)
text=apijs.i18n.translate('uploadOneType',last,maxsize);else
text=apijs.i18n.translate('uploadMultiType',this.extensions.slice(0,-1).join(', '),last,maxsize);return text;};this.showRetry=function(text,input){var callbackname=this.callback.name,title=document.getElementById('box').querySelector('h1').firstChild.nodeValue,button;apijs.dialog.dialogInformation(title,text,'eeupload');document.getElementById('iframeUpload').parentNode.removeChild(document.getElementById('iframeUpload'));if(callbackname!=='string'){callbackname=this.callback.toString().match(/function ([^\(]+)\(/)[0];callbackname=callbackname.slice(9,-1);}
button=document.getElementById('box').querySelector('button');button.firstChild.nodeValue=apijs.i18n.translate('buttonRetry');button.setAttribute('class','back');button.setAttribute('onclick',"apijs.upload.sendFile('"+title+"', '"+input+"', "+this.maxsize+", ['"+this.extensions.toString().replace(/,/g,"','")+"'], "+callbackname+", "+this.callbackParam+");");};this.actionConfirm=function(){var input=document.getElementById('box').getElementsByTagName('input')[1],filename=input.value.replace('C:\\fakepath\\',''),decimal,size=-1,maxsize=this.maxsize*1024*1024,text,last=this.extensions[this.extensions.length-1];if(filename.length<1){document.getElementById('box').getElementsByTagName('input')[1].focus();}
else if((last!=='*')&&!in_array(filename.slice(filename.lastIndexOf('.')+1).toLowerCase(),this.extensions)){if(filename.lastIndexOf('/')>0)
filename=filename.slice(filename.lastIndexOf('/')+1);else if(filename.lastIndexOf('\\')>0)
filename=filename.slice(filename.lastIndexOf('\\')+1);text=(this.extensions.length<2)?apijs.i18n.translate('uploadBadOneType',filename,last):apijs.i18n.translate('uploadBadMultiType',filename,this.extensions.slice(0,-1).join(', '),last);this.showRetry(text,input.getAttribute('name'));}
else{if(typeof input.files==='object')
size=input.files[0].size;if((size>-1)&&(size>maxsize)){if(maxsize>1048576){size=apijs.i18n.translate('uploadBadSizeM',(size/1048576).toFixed(2));maxsize=apijs.i18n.translate('uploadBadSizeM',(maxsize/1048576).toFixed(2));}
else{size=apijs.i18n.translate('uploadBadSizeK',(size/1024).toFixed(2));maxsize=apijs.i18n.translate('uploadBadSizeK',(maxsize/1024).toFixed(2));}
decimal=apijs.i18n.translate('uploadBadSizeDecimal');size=size.replace('.00','').replace('.',decimal);maxsize=maxsize.replace('.00','').replace('.',decimal);this.showRetry(apijs.i18n.translate('uploadBadSize',filename,size,maxsize),input.getAttribute('name'));}
else{document.getElementById('iframeUpload').setAttribute('onload','apijs.upload.endUpload();');this.percent=0;this.svgDirection=0;this.svgWaiting=10;this.progressWaiting=15;this.svgTimer=window.setInterval(apijs.upload.animGeneric.bind(this),50);this.progressTimer=window.setTimeout(apijs.upload.uploadRealTime,1000);window.setTimeout(function(){apijs.dialog.dialogProgress(document.getElementById('box').querySelector('h1').firstChild.nodeValue,apijs.i18n.translate('uploadInProgress'),apijs.dialog.classNames.replace(/upload|lock/,''));},1);return true;}}
return false;};this.actionDelete=function(fileid){var xhr=new XMLHttpRequest();xhr.onreadystatechange=function(){if((xhr.readyState===4)&&(xhr.status===200)){var result,status,message,icon,title=document.getElementById('box').querySelector('h1').firstChild.nodeValue;try{result=xhr.responseXML;status=result.getElementsByTagName('status')[0].firstChild.nodeValue;message=result.getElementsByTagName('message')[0].firstChild.nodeValue;icon=(status==='success')?'information':'error';apijs.dialog.dialogInformation(title,message,icon);if(status==='success')
apijs.upload.callback(apijs.upload.callbackParam);}
catch(ee){apijs.dialog.dialogInformation(title,'[pre]'+ee+'[/pre]','error');}}
else if((xhr.readyState===4)&&(xhr.status!==200)){apijs.dialog.dialogInformation(document.getElementById('box').querySelector('h1').firstChild.nodeValue,apijs.i18n.translate('deleteNotFound',xhr.status,xhr.statusText),'error');}};xhr.open('GET',apijs.config.dialog.fileUpload+'?delete='+fileid,true);xhr.send(null);};this.uploadRealTime=function(){var xhr=new XMLHttpRequest();xhr.onreadystatechange=function(){if((xhr.readyState===4)&&(xhr.status===200)&&(apijs.upload.progressWaiting>0)&&(apijs.upload.percent<100)){try{var result,status,percent,rate,time;result=xhr.responseXML;status=result.getElementsByTagName('status')[0].firstChild.nodeValue;if(status==='uploading'){percent=parseInt(result.getElementsByTagName('percent')[0].firstChild.nodeValue,10);rate=result.getElementsByTagName('rate')[0].firstChild.nodeValue;time=result.getElementsByTagName('time')[0].firstChild.nodeValue;if((percent>0)&&(percent<100)&&(percent>apijs.upload.percent)){if(apijs.upload.svgTimer)
...
clearInterval(this.svgTimer);if(this.extensions===null)
return;this.percent=100;this.maxsize=0;this.extensions=null;this.svgTimer=null;this.svgDirection=0;this.svgWaiting=0;this.progressTimer=null;this.progressWaiting=0;var result,status,message;try{if(apijs.config.navigator){result=window.frames['iframeUpload.'+this.uploadkey].document;status=result.querySelector('status').firstChild.nodeValue;message=result.querySelector('message').firstChild.nodeValue;}
...
if(status==='success'){this.animToValue(100);window.setTimeout(function(){this.callback(this.uploadkey,this.callbackParam);}.bind(this),1500);}
