...
var apijsMagento = new (function () {
	...
	this.sendFiles = function (title, action, onemax, allmax) {
		apijs.upload.sendFiles(title, action, 'myimage', onemax, allmax, 'jpg,jpeg,gif,png', apijsMagento.updateForm);
	};
			...
			if (xhr.readyState === 4) {
				if ([0, 200].has(xhr.status)) {
					if (xhr.responseText.indexOf('success-') === 0)
						apijsMagento.updateForm(xhr.responseText);
					else
						apijsMagento.error(xhr.responseText);
				}
				else {
					apijsMagento.error(xhr.status);
				}
	...
	this.deleteAttachment = function (action) {
		apijs.dialog.dialogConfirmation(apijs.i18n.translate(250), apijs.i18n.translate(251), apijsMagento.actionDeleteAttachment, action);
	};
	...
	this.deleteMedia = function (elem) {
		...
		apijs.dialog.dialogConfirmation(apijs.i18n.translate(250), apijs.i18n.translate(251), apijsMagento.actionDeleteMedia, elem.id);
	};

	this.actionDeleteMedia = function (args) {
	...
	this.clearCache = function (action) {
		...
		try {
			apijs.dialog.dialogConfirmation(apijs.i18n.translate(255), apijs.i18n.translate(256), apijsMagento.actionClearCache, action);
		}
		catch (e) {
			console.error(e);
...
if (typeof self.addEventListener == 'function')
	self.addEventListener('apijsload', apijsMagento.start.bind(apijsMagento));